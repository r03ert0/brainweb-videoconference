...
function onLocalTracks(tracks) {
  ...
  for (let i = 0; i < localTracks.length; i++) {
    ...
    if (localTracks[i].getType() === 'video') {
      $('body').append(`<div class="frame"><div class="video"><video autoplay='1' id='localVideo${i}' /></div></div>`);
...
function onRemoteTrack(track) {
  ...
  if (trackType === 'video') {
    if(el === null) {
      $('body').append(`<div class="frame"><div class="video"><video autoplay='1' id='${id}' /></div></div>`);
    }
  } else {
    if(el === null) {
